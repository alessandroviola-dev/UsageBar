...
| OpenRouter | API key in macOS Keychain | Documented non-billable `https://openrouter.ai/api/v1/credits`; purchased credits and used credits provide a denominator | FULL |
...
`FULL` means UsageBar can calculate a mathematically valid remaining percentage from a real quota limit. `PARTIAL` means a secure connection exists but a global percentage cannot truthfully be shown. `UNSUPPORTED` means no secure/reliable adapter is implemented. UsageBar does not make billable generation requests to discover limits.
